                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4 relative">
                            ...
                            <div class="border-t pt-4">
                                <p class="text-sm text-gray-500">Posted by {{ $advertisement->user->name }}</p>
                            </div>
                            ...
                            @elseif($advertisement->type === 'rental')
                                <div class="mt-6 space-y-4">
                                    <div class="bg-gray-50 p-4 rounded-lg">
                                        ...
                                        <p>Condition: {{ $advertisement->condition }}%</p>
                                        <p>Wear per day: {{ $advertisement->wear_per_day }}%</p>
                                    </div>
                                    <form action="{{ route('rentals.store') }}" method="POST">
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <input type="date" name="end_date" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                            </div>
                                            <button type="submit" class="w-full bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                                                Rent Now
                                            </button>
                            ...
                            @elseif($advertisement->type === 'auction')
                                <div class="mt-6 space-y-4">
                                    <div class="bg-gray-50 p-4 rounded-lg">
                                        <h3 class="font-semibold">Auction Details</h3>
                                        <p>Start Date: {{ \Carbon\Carbon::parse($advertisement->auction_start_date)->format(app()->getLocale() == 'nl' ? 'd-m-Y H:i' : 'F j, Y g:i A') }}</p>
                                        <p>End Date: {{ \Carbon\Carbon::parse($advertisement->auction_end_date)->format(app()->getLocale() == 'nl' ? 'd-m-Y H:i' : 'F j, Y g:i A') }}</p>
                                        @if($advertisement->bids->count() > 0)
                                            <p>Current Highest Bid: €{{ number_format($advertisement->bids->max('amount'), 2) }}</p>
                                        @endif
                                    </div>
                                    @if(now()->between($advertisement->auction_start_date, $advertisement->auction_end_date))
                                        <form action="{{ route('bids.store') }}" method="POST">
                                            ...
                                            <div class="space-y-4">
                                                <div>
                                                    <label class="block text-sm font-medium text-gray-700">Your Bid Amount</label>
                                                    <input type="number" name="amount" step="0.01" min="{{ $advertisement->price }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                                </div>
                                                <button type="submit" class="w-full bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                                                    Place Bid
                                                </button>
                                    ...
                                    @else
                                        <div class="text-center p-4 bg-gray-100 rounded-lg">
                                            @if(now() < $advertisement->auction_start_date)
                                                <p class="text-gray-600">Auction hasn't started yet</p>
                                            @else
                                                <p class="text-gray-600">Auction has ended</p>
                                            @endif
                                        </div>
                                    @endif
                                </div>
